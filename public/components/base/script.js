			...
			nodes.forEach( function ( node ) {
				new Dropzone( node, {
                    success: function(res) {
                        let len = parseInt($('.dz-preview').length);

                        let response = parseJSON(res.xhr.response);
                        $('.dz-preview:eq(' + (len - 1) + ')').find('.dz-image').append('<input class="dropzone-input" type="hidden" name="image[]" value="'+ response.data.path +'">');
                    },
					paramName: "file",
					addRemoveLinks: true,
